...
(define metadata
  '((title . "Vim plugins - HiPhish's Workshop")
    (css . ("/vim-plugins/extra.css"))))

(define-syntax-rule
  (define-vim-plugins (plugins-list-name urls-list-name description-list-name)
    (name url
      (description ...))
    ...)
  ...
  (define-values (plugins-list-name urls-list-name description-list-name)
    (values
      (list name ...)
      (list url ...)
      (list '(description ...) ...))))

(define-vim-plugins (vim-plugins vim-urls vim-descriptions)
...
(define-vim-plugins (nvim-plugins nvim-urls nvim-descriptions)
...
(define content
  `((h1 "Plugins for Vim and Neovim")
    ...
    (dl
      ,@(map vim-plugin->sxml vim-plugins vim-urls vim-descriptions))
    (h2 "Neovim only")
    (p "These plugins only work on Neovim. If anyone wants to submit a patch to
       make them work for Vim as well be my guest.")
    (dl
      ,@(map vim-plugin->sxml nvim-plugins nvim-urls nvim-descriptions))))
